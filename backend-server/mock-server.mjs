...
// (Removed noisy request logger to satisfy lint rules)
...
// States derived from cities
const mockStates = (() => {
  const uniq = new Map();
  for (const c of mockCities) {
    const key = (c.state || 'Unknown').toLowerCase();
    if (!uniq.has(key)) {
      uniq.set(key, {
        id: generateCoworsId(EntityType.STATE),
        name: c.state || 'Unknown',
        gst_state_code: c.gst_state_code || null,
...
// Areas (replacement for neighborhoods)
const mockAreas = [];
for (const city of mockCities) {
  mockAreas.push(
    { id: generateCoworsId(EntityType.AREA), city_id: city.id, name: `${city.name} Central`, display_name: `${city.name} Central`, popular_tags: ['central', 'business'], is_popular: true, created_at: new Date().toISOString(), updated_at: new Date().toISOString() },
    { id: generateCoworsId(EntityType.AREA), city_id: city.id, name: `${city.name} South`, display_name: `${city.name} South`, popular_tags: ['residential', 'quiet'], is_popular: false, created_at: new Date().toISOString(), updated_at: new Date().toISOString() },
    { id: generateCoworsId(EntityType.AREA), city_id: city.id, name: `${city.name} Tech Park`, display_name: `${city.name} Tech Park`, popular_tags: ['tech', 'startup'], is_popular: true, created_at: new Date().toISOString(), updated_at: new Date().toISOString() }
  );
}
// Areas serve as the replacement for neighborhoods; the old route maps to areas directly
...
app.get('/api/v1/states/:stateId/cities', (req, res) => {
  const { stateId } = req.params;
  const state = mockStates.find(s => s.id === stateId) || mockStates.find(s => s.name.toLowerCase() === decodeURIComponent(stateId).toLowerCase());
  if (!state) return res.status(404).json({ message: 'State not found' });
  const cities = mockCities.filter(c => (c.state || '').toLowerCase() === state.name.toLowerCase());
  res.json(cities);
});

app.get('/api/v1/cities/:cityId/areas', (req, res) => {
  const { cityId } = req.params;
  const list = mockAreas.filter(a => a.city_id === cityId);
  res.json(list);
});

app.get('/api/v1/states/:stateId/cities/:cityId/areas', (req, res) => {
  const { stateId, cityId } = req.params;
  const city = mockCities.find(c => c.id === cityId);
  if (!city) return res.status(404).json({ message: 'City not found' });
  const state = mockStates.find(s => s.id === stateId) || mockStates.find(s => s.name.toLowerCase() === decodeURIComponent(stateId).toLowerCase());
  if (!state || (city.state || '').toLowerCase() !== state.name.toLowerCase()) {
    return res.status(404).json({ message: 'State/city mismatch' });
  }
  const list = mockAreas.filter(a => a.city_id === cityId);
  res.json(list);
});

// Backward-compatible neighborhoods path
