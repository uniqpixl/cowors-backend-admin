...
export function TransactionActions({ transactionId }: TransactionActionsProps) {
  ...
  const handleSendMessage = () => {
    ...
    console.log(`Send message for transaction ${transactionId}`);
  };
      ...
      <div className="rounded-lg border border-stroke bg-white p-6 shadow-lg dark:border-strokedark dark:bg-boxdark">
        ...
        <div className="space-y-4">
          ...
          >
            ...
            <span className="font-semibold">Cancel Transaction</span>
          </button>
        </div>
      </div>
      ...
      {showRefundModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
          <div className="mx-4 w-full max-w-md rounded-lg bg-white p-6 dark:bg-boxdark">
            ...
            <p className="mb-6 text-body dark:text-bodydark">
              Are you sure you want to process a refund for this transaction? The amount will be refunded to the customer&apos;s original payment method.
            </p>
            <div className="flex gap-3">
              <button
