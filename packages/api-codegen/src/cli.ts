...
program
  .command('generate')
  .description('Generate Zod schemas and API client from OpenAPI spec')
...
program
  .option('-i, --input <url>', 'OpenAPI spec URL or file path')
  .option('-o, --output <path>', 'Output directory path')
  .option('-n, --name <name>', 'API client name')
  .option('-f, --filter <tag>', 'Filter endpoints by tag')
  .action(async (options) => {
    if (options.input && options.output && options.name) {
      // Run generate with provided options
      await program.parseAsync(['node', 'cli.js', 'generate', 
        '--input', options.input,
        '--output', options.output, 
        '--name', options.name,
